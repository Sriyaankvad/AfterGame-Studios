...
public class Weapon : MonoBehaviour
{
    ...
    {
        string json = Resources.Load<TextAsset>("Weapons/" + path).ToString();
        JsonUtility.FromJsonOverwrite(json, this); // sets weapon properties from .json file
    ...
    public void Attack(float attackIndex)
    {
        ...
        // checks to see if there is an attack in progess; if not, checks if this attack is part of a multihit attack, updates the attack and counters accordingly, then starts a coroutine to execute the attack
        if (!attackInProgress)
        {
            ...
            StartCoroutine(ExecuteDelayedAttack(attackToExecute, attackStartDelay[(int)attackToExecute], multihitAttackInProgress ? 0 : attackFinishDelay[(int)attackToExecute])); // if a multihit attack is in progress, then there is no attack finish delay
